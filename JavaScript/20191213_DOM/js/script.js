...
window.onload=function(){
	//윈도우라는 최상위 객체가 로딩이 완료되면 function(함수문)을 실행하라는 의미
	...
	$firstObj.style.backgroundColor= "#ffaaff"; //<h2 id="title" style="color=red; background-color=#ffaaff;">문서객체 모델 테스트</h2>
	$firstObj.innerHTML="<span>더보기</span>"; //<h2 id="title" style="color=red; background-color=#ffaaff;"><span>더보기</span></h2>
	$firstObj.innerHTML="<span>감추기</span>"; //<h2 id="title" style="color=red; background-color=#ffaaff;"><span>더보기</span></h2>
	...
	menuObj.getElementsByTagName("p")[1].style.background="#ffffaa";
	...
	var $mList_parent = $mList.parentNode; //parentNode는 선택자의 바로 상의 부모만을 선택하여 반환(노드(엔터값 포함))는 무시됨
	console.log($mList_parent);
	$mList_parent.style.backgroundColor="#aaffaa";

	var $mList_children = $mList_parent.children; //children은 선택자의 하위 자식요소를 모두 반환(HTML 코딩상의 엔터값이 무시됨) 실제 요소명만들 자식으로 저장)
	console.log($mList_children);
	$mList_children[3].style.listStyle="none";

	var $mList_childNodes = $mList_parent.childNodes; //childNodes는 선택자의 하위 요소를 모두 반환(HTML 코딩상의 엔터값 까지 자식 요소로 저장,엔터값에 의한 공백도 자식으로 저장 )
  ...
  /* 속성 변경 및 삭제, 추가 */
	...
	$sec_01.innerHTML = "<h4>상세 페이지 내용</h4>"; // 기존 하위 자식 요소를 생성
	/*이벤트 핸들러 적용하기 (click,mouseover,mouseout) */
