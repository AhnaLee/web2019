$(document).ready(function(){
	...
	// load() 이벤트 : 페이지가 로딩된 후에 이벤트를 발생시킨다.
	// 외부파일을 호출하여 로드 할 때 사용가능 (공통적인 요소인 header 와 footer를 호출하여 화면상에 html문서 형태로 가져 올수 있음. 단, 호스팅 서버를 활용하는 것이 필수항목)
 	$(".click_ev").click(function(){
 		$(".click_ev").text("click complete");
	});
 	...
 	// href 의 속성을 막는 방법 
 	// #1. 클릭이벤트를 종료하기 직전 return false; 선언 
 	// #2. 매개변수(e)를 선언하고 바로 하단에 e.preventDefault();
 	// #3. 태그 내부에 href 속성 값으로 javascript:void(0); 작성 

 	//클래스의 유무를 판단해서 클래스가 있으면 제거
 	$("#detail").click(function(){
 		var $has_class = $("#detail").hasClass("active");
 		//클래스가 존재하면 true, 없다면 flase 
 	...
 	$(".card_img").hover(function(){ //마우스가 영역 내부로 진입시 이벤트 발생(mouseenter)
 		$(".card_img").attr("src","img/card.png");
 	}, function(){ // 마우스가 영역 밖으로 빠져 나갔을때 이벤트 발생(mouseleave) 
 		$(".card_img").attr("src","img/ace.png");
 	});
